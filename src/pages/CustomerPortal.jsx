...
// Supports both: USB/Bluetooth HID scanners (keyboard wedge) and camera scanning
function BarcodeInput({ value, onChange, placeholder, bottleNum }) {
  ...
  const handleKeyDown = useCallback((e) => {
    ...
    if (e.key === 'Enter') {
      // Scanner finished — use the buffer
  ...
  async function startCamera() {
    ...
    try {
      ...
      setCameraError('Camera not available. Please use scanner or type manually.')
    }
  }
  ...
  return (
    <div style={{marginBottom:6}}>
      <div style={{position:'relative',display:'flex',gap:6}}>
        ...
        <div style={{position:'relative',flex:1}}>
          <div style={{
            ...
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M3 9V6a1 1 0 011-1h3M3 15v3a1 1 0 001 1h3M15 5h3a1 1 0 011 1v3M21 15v3a1 1 0 01-1 1h-3"/>
        ...
        <button
          type="button"
          onClick={e => { e.stopPropagation(); setScannerActive(p=>!p); setCameraActive(false) }}
        ...
        >
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
            <path d="M5 7H3a2 2 0 00-2 2v6a2 2 0 002 2h2m14-10h2a2 2 0 012 2v6a2 2 0 01-2 2h-2M7 7h10v10H7z"/>
          </svg>
        ...
        <button
          type="button"
          onClick={e => { e.stopPropagation(); cameraActive ? stopCamera() : startCamera(); setScannerActive(false) }}
    ...
    <Modal open onClose={onClose} title={`Mark Bottles as Collected — ${batch.id}`} large
      footer={
        <>
          ...
          <button className="btn btn-primary" onClick={()=>{onSubmit(selected,locations,bottleIds);onClose()}} disabled={!allHaveLocation}>
    ...
    >
      ...
      {selected.length > 0 && (
        <div style={{padding:'12px 14px',background:'#faf8f5',borderRadius:'var(--r)',border:'1px solid var(--border)',marginBottom:14}}>
          ...
          <div style={{display:'flex',gap:8}}>
            <input type="text" value={globalLoc} onChange={e=>setGlobalLoc(e.target.value)}
      ...
      <div style={{maxHeight:460,overflowY:'auto',paddingRight:4}}>
        {emptyBottles.map(bottle => {
          ...
          return (
            <div key={bottle.id}
              style={{padding:'10px 12px',borderRadius:'var(--r)',marginBottom:6,
                border:`1.5px solid ${isSel?'rgba(212,82,12,0.3)':'var(--border)'}`,
                background:isSel?'rgba(212,82,12,0.04)':'#fff',
                transition:'all 0.12s',
              }}
            >
              <div style={{display:'flex',alignItems:'center',gap:10,cursor:'pointer',marginBottom:isSel?10:0}} onClick={()=>toggle(bottle.id)}>
                <input type="checkbox" checked={isSel} onChange={()=>{}} style={{width:'auto',accentColor:'var(--orange)',flexShrink:0}}/>
                <div style={{flex:1}}>
  ...
  return (
    <Modal open onClose={onClose} title={`Send Bottles to VPS Lab — ${batch.id}`}
      footer={
        <>
          <button className="btn btn-ghost" onClick={onClose}>Cancel</button>
          <button className="btn btn-primary" onClick={()=>{onSubmit(selected,form);onClose()}} disabled={selected.length===0||!form.awb.trim()}>
  ...
  return (
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(90px,1fr))',gap:6}}>
      {bb.map(b => {
        ...
        return (
          <div key={b.id} title={`Bottle #${b.bottleNum}: ${b.status}${b.location?` — ${b.location}`:''}`} style={{padding:'8px 6px',borderRadius:'var(--r)',background:sc.bg,border:`1.5px solid ${sc.border}`,textAlign:'center'}}>
...
// ── Main CustomerPortal ───────────────────────────────────────────────────────
// ── Request Batch Modal (Customer Portal) ────────────────────────────────────
...
export default function CustomerPortal({ batches, setBatches, bottles, setBottles, lockedCustomer=null }) {
  ...
  // My pending requests (shown only to this customer)
  ...
  function acknowledgeReceipt(batchId) {
    ...
    if (batch) {
      ...
      // Mark linked approved request as fulfilled
      ...
      {myBatches.map(b => {
        ...
        return (
          <div className="card mb-4" key={b.id}>
            ...
            {b.stage===1 && s.total>0 && (
              <>
                <button className="btn btn-ghost btn-sm" onClick={()=>setExpandedBatch(isExpanded?null:b.id)} style={{marginBottom:isExpanded?12:0}}>
