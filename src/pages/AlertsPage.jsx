...
export default function AlertsPage({ alerts, batches }) {
  ...
  return (
    <div>
      ...
      {/* Banner if overdue batches exist */}
      {overdue.length > 0 && (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 28 }}>
        {alerts.map(a => (
          <div
            ...
            style={{
              borderLeft: `3px solid var(--${a.severity === 'red' ? 'red' : 'amber'})`,
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              <div className="flex items-center gap-3 mb-1" style={{ flexWrap: 'wrap' }}>
                ...
                <span className={`badge badge-${a.severity === 'red' ? 'red' : 'amber'}`}>
                  {SEVERITY_LABELS[a.type] || a.type}
                </span>
              </div>
              <div style={{ fontSize: 13.5, color: 'var(--text-secondary)', lineHeight: 1.5, marginBottom: 10 }}>
                {a.msg}
              </div>
              <div className="flex gap-2" style={{ flexWrap: 'wrap' }}>
                <button className="btn btn-ghost btn-sm">
                  <Icons.Mail /> Send Reminder
                </button>
            ...
            <tbody>
              {overdue.map(b => (
                <tr key={b.id}>
                  ...
                  <td><span className="badge badge-red">{daysSince(b.dispatched)}d</span></td>
