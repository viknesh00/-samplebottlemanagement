...
import { Modal, SearchBar, SegBar, PriorityBadge, useSortPage, SortTh, Pagination, DatePicker } from '../components/UI'
import {
    ...
    <>
      <div className="grid-2">
        ...
        <div className="form-group">
          ...
          <DatePicker value={form.dispatched} onChange={v => up('dispatched', v)} />
        </div>
      </div>
      <div className="form-group">
...
/* ── Collapsible Customer Requests Panel ────────────────────────────────────── */
function RequestsPanel({ pendingRequests, allReviewed, setApproveReq, handleReject }) {
    ...
    <div className="card" style={{ marginBottom: 20, overflow: 'hidden' }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <ClipboardList size={16} color="var(--accent)" strokeWidth={2} />
          <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)' }}>Customer Batch Requests</span>
          {pendingRequests.length > 0 && (
            <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 9px', borderRadius: 20,
              background: 'rgba(212,42,42,0.1)', color: 'var(--red)', border: '1px solid rgba(212,42,42,0.25)',
      ...
      {open && (
        <>
          <div style={{ display:'flex', gap:2, padding:'0 20px', borderBottom:'1px solid var(--border)' }}>
            {reqTabs.map(t => (
              <button key={t.key} onClick={() => setReqTab(t.key)} style={{
                padding:'8px 16px', fontSize:11.5, fontWeight:600, background:'none', border:'none', cursor:'pointer',
                ...
                {t.count > 0 && (
                  <span style={{ fontSize:9.5, fontWeight:700, padding:'1px 6px', borderRadius:10,
                    ...
                    color: reqTab===t.key ? '#fff' : 'var(--text-muted)' }}>{t.count}</span>
                )}
              </button>
            ))}
          </div>
            ...
            pendingRequests.length === 0 ? (
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:10, padding:'14px 16px' }}>
                {pendingRequests.map(req => {
                  ...
                  return (
                    <div key={req.id} style={{ borderRadius:'var(--r)', border:`1px solid var(--border)`,
                      borderLeft:`3px solid ${pColor}`, background:'var(--surface)', overflow:'hidden' }}>
                      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'13px 16px' }}>
                        <div style={{ flex:1, minWidth:0 }}>
                          ...
                          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'2px 20px', fontSize:11.5 }}>
                            <div style={{ color:'var(--text-muted)' }}>Sample: <span style={{ color:'var(--text-primary)', fontWeight:600 }}>{req.sampleType}</span></div>
                            <div style={{ color:'var(--text-muted)' }}>Bottles: <span style={{ color:'var(--accent)', fontWeight:700, fontSize:13, fontFamily:'var(--font-display)' }}>{req.qty}</span></div>
                            <div style={{ color:'var(--text-muted)' }}>Requested: <span style={{ color:'var(--text-primary)', fontWeight:600 }}>{fmtDate(req.requestedDate)}</span></div>
          ...
          {reqTab === 'history' && (
            allReviewed.length === 0 ? (
              <div style={{ padding:'24px 20px', textAlign:'center', color:'var(--text-muted)', fontSize:12.5 }}>No reviewed requests yet.</div>
            ) : (
...
/* ── Batch Detail Modal ─────────────────────────────────────────────────────── */
function BatchDetailModal({ batch, onClose, onToggleStage }) {
  if (!batch) return null
  const isReceived = batch.stage === 1
  const priorityColor = batch.priority === 'urgent' ? 'var(--red)' : batch.priority === 'high' ? 'var(--amber)' : 'var(--text-muted)'
  const priorityBg    = batch.priority === 'urgent' ? 'rgba(212,42,42,0.08)' : batch.priority === 'high' ? 'rgba(201,122,6,0.08)' : 'var(--bg)'
  const days = batch.dispatched ? daysSince(batch.dispatched) : 0

  const field = (label, value, accent) => (
    <div>
      <div style={{ fontSize: 10, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.4px', marginBottom: 3 }}>{label}</div>
      <strong style={accent ? { color: accent } : {}}>{value || '—'}</strong>
    </div>
  )

  return (
    <Modal open onClose={onClose} title={`Batch Details — ${batch.id}`} large>
      <div style={{ padding: '10px 14px', borderRadius: 'var(--r)', marginBottom: 18,
        background: isReceived ? 'rgba(10,124,82,0.06)' : 'rgba(201,122,6,0.06)',
        border: `1px solid ${isReceived ? 'rgba(10,124,82,0.2)' : 'rgba(201,122,6,0.2)'}`,
        display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: isReceived ? 'var(--green)' : 'var(--amber)',
          textTransform: 'uppercase', letterSpacing: '0.5px' }}>
          {batch.issues?.length > 0 ? '⚠ Has Issues' : isReceived ? '✓ Received at Lab' : '⏳ Awaiting Receipt'}
        </div>
        {batch.issues?.length > 0 && (
          <div style={{ fontSize: 11, color: 'var(--red)', marginLeft: 8 }}>{batch.issues.join(' · ')}</div>
        )}
        <span style={{ marginLeft: 'auto', fontSize: 10.5, fontFamily: 'var(--font-mono)',
          color: days > 30 ? 'var(--red)' : 'var(--text-muted)' }}>{days} days since dispatch</span>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px 28px', fontSize: 12.5, marginBottom: 18 }}>
        {field('Customer', batch.customer?.split(' ').slice(0, 4).join(' '))}
        {field('Contact', batch.contact)}
        {field('Sample Type', batch.sampleType)}
        {field('Bottle Quantity', batch.qty, 'var(--accent)')}
        {field('Courier', batch.courier)}
        {field('AWB / Tracking', batch.awb)}
        {field('Dispatch Date', fmtDate(batch.dispatched))}
        {field('Received Date', batch.receivedDate ? fmtDate(batch.receivedDate) : 'Not yet received', batch.receivedDate ? 'var(--green)' : undefined)}
        <div>
          <div style={{ fontSize: 10, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.4px', marginBottom: 3 }}>Priority</div>
          <span style={{ fontSize: 10.5, fontWeight: 700, padding: '2px 9px', borderRadius: 20,
            background: priorityBg, color: priorityColor, border: `1px solid ${priorityColor}30`, textTransform: 'capitalize' }}>
            {batch.priority || 'normal'}
          </span>
        </div>
        {field('Collection Location', batch.location)}
        {batch.notes && (
          <div style={{ gridColumn: '1/-1' }}>
            <div style={{ fontSize: 10, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.4px', marginBottom: 3 }}>Notes</div>
            <span style={{ color: 'var(--text-secondary)', lineHeight: 1.5 }}>{batch.notes}</span>
          </div>
        )}
      </div>

      <div style={{ display: 'flex', gap: 10, justifyContent: 'space-between', paddingTop: 16, borderTop: '1px solid var(--border)' }}>
        <button className="btn btn-ghost" onClick={onClose}>Close</button>
        <button
          className={`btn ${isReceived ? 'btn-ghost' : 'btn-primary'}`}
          style={isReceived ? { borderColor: 'var(--amber)', color: 'var(--amber)' } : {}}
          onClick={() => { onToggleStage(batch.id); onClose() }}
        >
          {isReceived ? '↩ Mark as Awaiting' : '✓ Mark as Received'}
        </button>
      </div>
    </Modal>
  )
...
export default function Batches({ batches, setBatches, bottles, setBottles, customers, batchRequests = [], setBatchRequests }) {
  ...
  const preFiltered = useMemo(() => batches
    .filter(b => {
  ...
  const { paged, sortKey, sortDir, toggleSort, page, setPage, totalPages } =
    useSortPage(preFiltered, { key: 'dispatched', dir: 'desc' }, pageSize)
  ...
  function handleApprove(req, form) {
    ...
    const newBatch = {
      id: `VPS-${year}-${String(Math.floor(Math.random() * 900 + 100))}`,
      customer: req.customer, contact: req.contactName || '',
      qty: form.qty, stage: 0, dispatched: form.dispatched, receivedDate: null,
      courier: form.courier, awb: form.awb, priority: req.priority,
      sampleType: req.sampleType, location: req.location,
      notes: (req.notes || '') + (form.notes ? ` | Staff: ${form.notes}` : ''),
      issues: [],
    }
    setBatches(p => [...p, newBatch])
    setBatchRequests(p => p.map(r => r.id === req.id
      ? { ...r, status: 'Approved', approvedDate: today(), batchId: newBatch.id } : r))
  }

  function handleReject(reqId) {
    setBatchRequests(p => p.map(r => r.id === reqId
      ? { ...r, status: 'Rejected', rejectedDate: today() } : r))
  }

  function toggleStage(batchId) {
    setBatches(p => p.map(b => {
      if (b.id !== batchId) return b
      const newStage = b.stage === 0 ? 1 : 0
      return { ...b, stage: newStage, receivedDate: newStage === 1 ? today() : null }
    }))
  }
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 18 }}>
        {[
          ...
          <div key={s.label} style={{ flex: '1 1 140px', padding: '13px 15px',
      ...
      {batchRequests.length > 0 && (
        <RequestsPanel pendingRequests={pendingRequests} allReviewed={allReviewed}
          setApproveReq={setApproveReq} handleReject={handleReject} />
      )}
      ...
      <div className="table-wrap">
        <table>
          <thead>
            <tr>
              ...
              <TH label="Sample Type"  sk="sampleType" />
              <TH label="Qty"          sk="qty" />
              <TH label="Dispatched"   sk="dispatched" />
              <TH label="Courier"      sk="courier" />
              <th>Stage</th>
              <th>Bottle Progress</th>
              <TH label="Priority"     sk="priority" />
              <TH label="Days"         sk="dispatched" />
              <th>Actions</th>
            </tr>
          </thead>
          <tbody>
            {paged.map(b => {
              ...
              return (
                <tr key={b.id}>
                  <td><span className="mono text-accent" style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: '0.3px' }}>{b.id}</span></td>
                  <td>
                  ...
                  <td><span style={{ fontWeight: 800, fontSize: 15, fontFamily: 'var(--font-display)', color: 'var(--accent)' }}>{b.qty}</span></td>
                  <td>
                    <div style={{ fontSize: 11 }}>{fmtDate(b.dispatched)}</div>
                    {b.receivedDate && <div style={{ fontSize: 9.5, color: 'var(--green)', fontFamily: 'var(--font-mono)', marginTop: 2 }}>Rcvd {fmtDate(b.receivedDate)}</div>}
                  </td>
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: 5 }}>
                      <button className="btn btn-ghost btn-sm btn-icon" title="View Details" onClick={() => setViewBatch(b)}><Icons.Eye /></button>
                      <button
                        className="btn btn-ghost btn-sm btn-icon"
                        title={b.stage === 0 ? 'Mark as Received' : 'Mark as Awaiting'}
                        style={b.stage === 1 ? { color: 'var(--amber)' } : {}}
                        onClick={() => toggleStage(b.id)}
                      ><Icons.Return /></button>
                    </div>
                  </td>
                </tr>
        ...
        <Pagination page={page} totalPages={totalPages} onPage={setPage} total={preFiltered.length} pageSize={pageSize} onPageSizeChange={setPageSize} />
      </div>
