...
import { StageBadge, EmptyState, Modal } from '../components/UI'
import * as Icons from '../components/Icons'

// ── Inbound Receipt Modal ─────────────────────────────────────────────────────
// VPS staff fills this when physically receiving returned samples
function ReceiptModal({ batch, onClose, onConfirm }) {
  ...
  function handleConfirm() {
    ...
    // Auto-flag insufficient quantity
    ...
    // Auto-flag wrong location if staff typed something different
    if (
      actualLocation.trim() &&
  ...
  return (
    <Modal
      open
      onClose={onClose}
      title={`Receive Inbound Samples — ${batch.id}`}
      footer={
        <>
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{
            width: 22, height: 22, borderRadius: '50%', background: 'var(--border)',
        ...
        <div className="grid-2" style={{ gap: 12 }}>
          ...
          <div>
            ...
            <input
              ...
              style={{
                width: '100%', background: qtyReceived < batch.qty ? 'rgba(239,68,68,0.06)' : 'var(--surface)',
        ...
        {qtyReceived < batch.qty && (
          <div style={{
            marginTop: 10, padding: '8px 12px', background: 'rgba(239,68,68,0.07)',
            border: '1px solid rgba(239,68,68,0.2)', borderRadius: 8,
            fontSize: 12.5, color: 'var(--red)', display: 'flex', gap: 6, alignItems: 'center',
          }}>
            <Icons.Warn />
            <span>
              <strong>Insufficient quantity</strong> — {batch.qty - qtyReceived} bottle{batch.qty - qtyReceived > 1 ? 's' : ''} missing.
              This will be auto-flagged.
            </span>
          </div>
        )}
      </div>
      ...
      <div style={{
        ...
        <div className="form-group" style={{ marginBottom: 8 }}>
          <label style={{ fontSize: 12, color: 'var(--text-muted)' }}>
            Actual location written on bottle label / accompanying form
          </label>
          <input
            type="text"
            ...
            style={{
              ...
              borderRadius: 'var(--radius)', padding: '8px 12px', fontSize: 13.5,
              color: 'var(--text-primary)', outline: 'none', marginTop: 4,
            }}
          />
        </div>
        {actualLocation.trim() &&
          ...
          <div style={{
            ...
            <Icons.Warn />
            <span>
              <strong>Location mismatch</strong> — sample collected from a different location than instructed.
              This will be auto-flagged.
            </span>
          </div>
        )}
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {[
            'Bottles damaged or leaking',
            'Samples appear contaminated',
            'Labels missing or unreadable',
            'Samples collected from wrong equipment type',
            ...
            <label key={issue} style={{
              ...
              />
              <span style={{ fontSize: 13, color: issues.includes(issue) ? 'var(--text-primary)' : 'var(--text-secondary)' }}>
                {issue}
              </span>
            </label>
          ))}
      ...
      {/* Step 4 — Free-text note */}
      <div className="form-group">
        <label style={{ fontSize: 12, color: 'var(--text-muted)' }}>Additional note (optional)</label>
        <textarea
          value={note}
          onChange={e => setNote(e.target.value)}
          placeholder="Any other observations on receipt…"
          style={{
            width: '100%', background: '#ffffff', border: '1px solid #e2e8f0',
            borderRadius: 'var(--radius)', padding: '8px 12px', fontSize: 13.5,
            color: 'var(--text-primary)', outline: 'none', resize: 'vertical', minHeight: 60,
            marginTop: 4,
          }}
        />
      </div>

      {/* Summary of what will be flagged */}
      {hasIssue && (
        <div style={{
          ...
          <ul style={{ paddingLeft: 16, lineHeight: 1.8 }}>
            {qtyReceived < batch.qty && (
              <li>Insufficient quantity ({qtyReceived}/{batch.qty} bottles received)</li>
            )}
            {actualLocation.trim() && actualLocation.trim().toLowerCase() !== expectedLocation.trim().toLowerCase() && (
              <li>Wrong collection location</li>
            )}
...
export default function Dispatch({ batches, setBatches }) {
  ...
  const [receiving, setReceiving] = useState(null) // batch being received
    ...
    <div>
      <div className="mb-6">
        ...
        <div className="section-sub">
          Manage courier movements — outbound empty bottles &amp; inbound sample returns
        </div>
      </div>
      ...
      <div className="tab-bar">
        {['outbound', 'inbound'].map(t => (
          ...
          >
            ...
            {t === 'inbound' && inbound.length > 0 && (
              <span className="nav-badge" style={{ marginLeft: 8 }}>{inbound.length}</span>
            )}
          </button>
        ))}
      </div>
      ...
      {tab === 'outbound' && (
        <>
          <div className="alert alert-teal mb-4">
            ...
            <div>
              <strong>Outbound:</strong> Empty bottles being sent from VPS Lab to customers.
              Mark as dispatched after courier pickup.
            </div>
          </div>
          ...
          <div className="table-wrap">
            <table>
              <thead>
                <tr>
                  <th>Batch</th><th>Customer</th><th>Qty</th>
                  <th>Courier</th><th>AWB</th><th>Stage</th><th>Action</th>
                </tr>
              </thead>
              <tbody>
                {outbound.map(b => (
                  <tr key={b.id}>
                    ...
                    <td>
                      {b.stage === 0 && (
                        <button className="btn btn-primary btn-sm" onClick={() => mark(b.id, 1)}>
                          Mark Dispatched
                        </button>
                      )}
                      {b.stage === 1 && (
                        <span className="badge badge-gray">Awaiting receipt</span>
                      )}
                    </td>
                  </tr>
                ))}
                {!outbound.length && (
                  <tr><td colSpan={7}>
                    <EmptyState icon={Icons.Truck} message="No outbound dispatches pending" />
                  </td></tr>
                )}
              </tbody>
            </table>
      ...
      {tab === 'inbound' && (
        <>
          <div className="alert alert-amber mb-4">
            ...
            <div>
              <strong>Inbound:</strong> Click <strong>"Receive & Inspect"</strong> to log the
              physical receipt. You will be prompted to verify bottle count and collection
              location — any discrepancy is automatically flagged.
            </div>
          </div>
          ...
          <div className="table-wrap">
            <table>
              <thead>
                <tr>
                  <th>Batch</th><th>Customer</th><th>Qty Sent</th>
                  <th>Expected Location</th><th>Existing Issues</th><th>Stage</th><th>Action</th>
                </tr>
              </thead>
              <tbody>
                {inbound.map(b => (
                  <tr key={b.id}>
                    ...
                    <td style={{
                      ...
                      whiteSpace: 'nowrap', fontSize: 12.5, color: 'var(--text-muted)',
                    }}>
                    ...
                    <td>
                      {b.stage === 5 && (
                        <button
                          ...
                          onClick={() => setReceiving(b)}
                        >
                          Receive &amp; Inspect
                        </button>
                      )}
                      {b.stage === 4 && (
                        <span className="badge badge-gray">In transit</span>
                      )}
                    </td>
                  </tr>
                ))}
                {!inbound.length && (
                  <tr><td colSpan={7}>
                    <EmptyState icon={Icons.Bottle} message="No inbound returns currently" />
                  </td></tr>
                )}
              </tbody>
            </table>
