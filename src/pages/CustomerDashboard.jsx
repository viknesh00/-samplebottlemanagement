...
const STATUS_STEPS = [
  { key: 'Empty',        label: 'Bottles Received',  icon: '📦', color: '#6b7280' },
  { key: 'Collected',    label: 'Samples Collected',  icon: '🧪', color: '#2563eb' },
  { key: 'Sent to VPS',  label: 'Sent to VPS Lab',   icon: '🚚', color: '#d97706' },
  { key: 'In Lab',       label: 'In Lab Testing',     icon: '🔬', color: '#7c3aed' },
  { key: 'Tested',       label: 'Testing Complete',   icon: '✅', color: '#0891b2' },
  { key: 'Report Ready', label: 'Report Ready',       icon: '📄', color: '#059669' },
]

// ── Request Batch Modal ───────────────────────────────────────────────────────
function RequestBatchModal({ customerName, onClose, onSubmit }) {
  const [form, setForm] = useState({
    sampleType: 'Transformer Oil',
    qty: 12,
    location: '',
    notes: '',
    priority: 'normal',
...
function StatCard({ icon, value, label, sub, color, bg, onClick }) {
  return (
    <div
      onClick={onClick}
      style={{
        padding:'18px 20px', borderRadius:'var(--r)', background: bg || '#fff',
...
function PipelineBar({ batches, bottles }) {
...
function RequestHistoryTable({ myRequests }) {
  ...
  return (
    <div className="card mb-6">
      ...
      {/* Tabs */}
      <div style={{display:'flex',gap:2,padding:'0 20px 0',borderBottom:'1px solid var(--border)',marginBottom:0}}>
        {tabs.map(t => (
          <button
            key={t.key}
...
export default function CustomerDashboard({ batches, bottles, reports, batchRequests, setBatchRequests, customerName }) {
  ...
  const myReports      = issuedReports.length

  // Only look at THIS customer's requests
  ...
  // Block new request if one is already pending
  ...
  return (
    <div>
        ...
        <div style={{
          ...
          <div style={{flex:1}}>
            <div style={{fontWeight:700,fontSize:13,color:'#92400e'}}>
              Your batch request is awaiting VPS Lab approval
            </div>
            <div style={{fontSize:12,color:'#a16207',marginTop:2}}>
              {pendingReqs[0].qty} × {pendingReqs[0].sampleType} · {pendingReqs[0].location}
            </div>
            <div style={{fontSize:11,color:'#b45309',marginTop:2}}>
              You can submit a new request once this one is approved or rejected.
            </div>
          </div>
        ...
        <div style={{
          ...
          <div style={{flex:1}}>
            <div style={{fontWeight:700,fontSize:13,color:'#065f46'}}>
              Batch request approved! Bottles are being dispatched.
            </div>
            <div style={{fontSize:12,color:'#047857',marginTop:2}}>Check My Batches to acknowledge receipt.</div>
          </div>
          <button className="btn btn-ghost btn-sm" style={{borderColor:'var(--green)',color:'var(--green)'}} onClick={()=>navigate('/portal')}>
            View Batches →
          </button>
        </div>
      )}
      ...
      <div className="grid-4 mb-6">
        <StatCard icon="📦" value={activeBatches}  label="Pending Acknowledgement" sub="Tap to acknowledge"       color="#d97706" bg="#fffbeb" onClick={()=>navigate('/portal')} />
        <StatCard icon="🚚" value={inTransit}       label="Bottles In Transit"      sub="On the way to VPS Lab"    color="#7c3aed" bg="#f5f3ff" onClick={()=>navigate('/portal')} />
        <StatCard icon="🔬" value={inLab}           label="Bottles In Lab"          sub="Being tested at VPS"      color="#0891b2" bg="#ecfeff" />
        <StatCard icon="📄" value={myReports}       label="Reports Available"        sub={reportReady>0?`${reportReady} bottles report ready`:''} color="#059669" bg="#f0fdf4" onClick={()=>navigate('/portal')} />
      </div>
        ...
        <div className="card">
          ...
          {latestBatch ? (
              ...
              <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:16,padding:'10px 14px',background:'#faf8f5',borderRadius:'var(--r)',border:'1px solid var(--border)'}}>
                ...
                <div style={{marginLeft:'auto'}}>
                  {latestBatch.stage === 0 ? (
                    ...
                    <span style={{padding:'4px 10px',borderRadius:20,background:'#f0fdf4',color:'#065f46',fontSize:11,fontWeight:700,border:'1px solid #a7f3d0'}}>✓ Received</span>
                  )}
                </div>
              </div>
              {latestStats && (
                <PipelineBar batches={[latestBatch]} bottles={bottles.filter(b=>b.batchId===latestBatch.id)}/>
              )}
            </>
          ) : (
            <div style={{textAlign:'center',padding:'28px',color:'var(--text-muted)'}}>
              ...
              <div style={{fontSize:11,color:'var(--text-muted)',marginTop:6}}>Go to My Batches &amp; Samples to request a new batch.</div>
            </div>
          )}
        </div>

        {/* All bottles pipeline */}
        <div className="card">
          <div className="card-header">
          ...
          {bottles.length > 0 ? (
            <>
              <PipelineBar batches={batches} bottles={bottles}/>
      ...
      <div className="card mb-6">
        ...
        {issuedReports.length === 0 ? (
          <div style={{textAlign:'center',padding:'24px',color:'var(--text-muted)',fontSize:13}}>No reports yet — reports will appear here once testing is complete.</div>
        ) : (
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:12}}>
            {issuedReports.map(r => {
              const rc = r.result==='Normal'?{bg:'#f0fdf4',color:'#059669',border:'#a7f3d0'}:r.result==='Warning'?{bg:'#fffbeb',color:'#d97706',border:'#fde68a'}:{bg:'#fef2f2',color:'#dc2626',border:'#fecaca'}
              return (
                <div key={r.id} style={{padding:'14px 16px',borderRadius:'var(--r)',border:`1.5px solid ${rc.border}`,background:rc.bg}}>
                  ...
                  {(() => {
                    const barcodes = bottles.filter(b => r.bottleIds?.includes(b.id) && b.bottleBarcode).map(b => b.bottleBarcode)
      ...
      {/* Request history — professional tabbed table */}
