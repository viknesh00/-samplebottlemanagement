...
function ResultForm({ count, onCancel, onSubmit }) {
  ...
  return (
    <div>
      <div style={{display:'flex',alignItems:'center',gap:10,padding:'12px 16px',background:'#faf8f5',borderRadius:'var(--r)',marginBottom:20,border:'1px solid var(--border)'}}>
      ...
      <div className="flex gap-3 mt-4">
        ...
        >
          <Icons.Reports/> Generate Report & Complete
        </button>
      </div>
    </div>
  )
...
export default function Lab({ bottles, setBottles, batches, reports, setReports }) {
  const [selected, setSelected]       = useState([])   // bottle IDs selected for testing
  const [showResults, setShowResults] = useState(false) // modal open

  // Inbound: sent by customer, not yet received by lab
  const inbound = useMemo(()=>
    bottles.filter(b=>b.status==='Sent to VPS'),
    [bottles]
  )

  // In-lab queue: received, awaiting testing
  const inQueue = useMemo(()=>
    bottles.filter(b=>b.status==='In Lab'),
    [bottles]
  )

  // Keep selected in sync — only show IDs that are still in the queue
  ...
  // Receive all bottles for a batch
  ...
  // Toggle individual bottle selection
  function toggleBottle(id) {
    setSelected(p=>p.includes(id) ? p.filter(x=>x!==id) : [...p,id])
  }

  // Toggle all bottles in a batch group
  ...
  // Submit test results → update bottles + create report
  function submitResults(form) {
    if (activeSelected.length === 0) return
    const reportId = uid('RPT')
    const dateStr  = new Date().toISOString().slice(0,10)

    // Collect all unique batchIds among selected bottles
    ...
    // Build a combined customer label if bottles span multiple batches
  ...
  return (
    <div>
      ...
      <div className="grid-2" style={{gap:20,alignItems:'start'}}>
            ...
            Object.entries(queueByBatch).map(([batchId,{batch,bottles:bb}])=>{
              ...
              return (
                <div key={batchId} style={{marginBottom:14}}>
                  {/* Batch header row — click to toggle all */}
                  <div
        ...
        {/* Right panel: selection summary + recent reports */}
        <div>
          {activeSelected.length === 0 ? (
